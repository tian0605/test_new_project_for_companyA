...
const OfflineMeterCarbon = ({ setRedirect, setRedirectUrl, t }) => {
    ...
    fetch(
      ...
      .then(json => {
        if (isResponseOK) {
          ...
          setReportingPeriodEnergyCarbonRate(
  ...
  const handleExport = e => {
    ...
    const fileName = 'offlinemetercarbon.xlsx';
    ...
    <Fragment>
      <div>
        <Breadcrumb>
          ...
          <BreadcrumbItem active>{t('Offline Meter Carbon')}</BreadcrumbItem>
        </Breadcrumb>
      </div>
...
export default withTranslation()(withRedirect(OfflineMeterCarbon));