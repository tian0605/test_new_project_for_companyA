        ...
        try:
            ...
            try:
                ...
                add_values = (" INSERT INTO tbl_spaces "
                              "    (name, uuid, parent_space_id, area, number_of_occupants, "
                              "     timezone_id, is_input_counted, "
        ...
        try:
            ...
            try:
                ...
                update_row = (" UPDATE tbl_spaces "
                              " SET name = %s, parent_space_id = %s, area = %s, "
                              "     number_of_occupants = %s, timezone_id = %s, "
    ...
    def on_post(req, resp, id_):
        ...
        try:
            ...
            try:
                ...
                add_row = (" INSERT INTO tbl_spaces_energy_storage_power_stations "
                           " (space_id, energy_storage_power_station_id) "
                           " VALUES (%s, %s) ")
    ...
    def on_delete(req, resp, id_, eid):
        ...
        try:
            ...
            try:
                ...
                cursor.execute(" DELETE FROM tbl_spaces_equipments WHERE space_id = %s AND equipment_id = %s ",
                               (id_, eid))
                cnx.commit()
            finally:
                if cursor:
        ...
        try:
            ...
            try:
                ...
                query = (" SELECT e.id, e.name, e.uuid, e.phase_of_lifecycle "
                         ...
                         "      tbl_photovoltaic_power_stations e "
    ...
    def on_post(req, resp, id_):
        ...
        try:
            ...
            try:
                ...
                add_row = (" INSERT INTO tbl_spaces_photovoltaic_power_stations "
                           " (space_id, photovoltaic_power_station_id) "
                           " VALUES (%s, %s) ")
                cursor.execute(add_row, (id_, photovoltaic_power_station_id,))
                cnx.commit()

                resp.status = falcon.HTTP_201
                resp.location = ('/spaces/' + str(id_) +
                                 '/photovoltaicpowerstations/' + str(photovoltaic_power_station_id))
            finally:
                if cursor:
                    cursor.close()
        finally:
    ...
    def on_delete(req, resp, id_, sid):
        ...
        try:
            ...
            try:
                ...
                cursor.execute(" DELETE FROM tbl_spaces_shopfloors WHERE space_id = %s AND shopfloor_id = %s ",
                               (id_, sid))
                cnx.commit()
            finally:
                if cursor:
        ...
        try:
            ...
            try:
                ...
                if row is None:
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "uuid": row[2]}
                            command_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "uuid": row[2],
                                           "energy_category": energy_category_dict.get(row[3], None)}
                            meter_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "uuid": row[2],
                                           "energy_category": energy_category_dict.get(row[3], None)}
                            offlinemeter_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "uuid": row[2],
                                           "energy_category": energy_category_dict.get(row[3], None)}
                            virtualmeter_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "uuid": row[2]}
                            shopfloor_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "uuid": row[2]}
                            combinedequipment_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "uuid": row[2]}
                            equipment_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1],
                                           "data_source": data_source_dict.get(row[2], None)}
                            point_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "uuid": row[2]}
                            sensor_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "uuid": row[2]}
                            tenant_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "uuid": row[2]}
                            store_result.append(result_item)
                    ...
                    if rows is not None and len(rows) > 0:
                        for row in rows:
                            result_item = {"id": row[0], "name": row[1], "description": row[2]}
                            workingcalendar_result.append(result_item)
    ...
    def on_post(req, resp):
        ...
        try:
            ...
            try:
                ...
                add_values = (" INSERT INTO tbl_spaces "
                              "    (name, uuid, parent_space_id, area, timezone_id, "
                              "     is_input_counted, is_output_counted, "
                              "     contact_id, cost_center_id, latitude, longitude, description) "
