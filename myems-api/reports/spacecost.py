"""
Space Cost Report API

This module provides REST API endpoints for generating space cost reports.
It analyzes energy costs associated with space energy consumption, providing insights
into cost optimization opportunities and financial performance metrics.

Key Features:
- Space energy cost analysis
...
from reports.productreporting import ensure_request_space_visible, ensure_space_product_bound, \
    get_space_product_energy_sources, get_energy_category_ids_for_sources, get_dimension_hourly_rows
    ...
    def on_get(req, resp):
        ...
        try:
            ...
            try:
                ...
                product_energy_sources = get_space_product_energy_sources(cursor_system, int(space['id']), product_id)
                base_category_hourly_rows = get_dimension_hourly_rows(cursor_billing,
                                                                     product_energy_sources,
                                                                     base_start_datetime_utc,
                                                                     base_end_datetime_utc)
                reporting_category_hourly_rows = get_dimension_hourly_rows(cursor_billing,
                                                                          product_energy_sources,
                                                                          reporting_start_datetime_utc,
                ...
                # Step 3: query energy categories
                ###################################################################################################
                energy_category_set = get_energy_category_ids_for_sources(product_energy_sources)
                ...
                if energy_category_set is not None and len(energy_category_set) > 0:
                    for energy_category_id in energy_category_set:
                        ...
                        rows_space_hourly = base_category_hourly_rows.get(energy_category_id, [])
                ...
                if energy_category_set is not None and len(energy_category_set) > 0:
                    for energy_category_id in energy_category_set:
                        ...
                        rows_space_hourly = reporting_category_hourly_rows.get(energy_category_id, [])
                ...
                if energy_category_set is not None and len(energy_category_set) > 0:
                    for energy_category_id in energy_category_set:
                        ...
                        for child_space in child_space_list:
                            ...
                            child_space_sources = get_space_product_energy_sources(cursor_system,
                                                                                  child_space['id'],
                                                                                  product_id)
                            child_space_rows = get_dimension_hourly_rows(cursor_billing,
                                                                         child_space_sources,
                                                                         reporting_start_datetime_utc,
                                                                         reporting_end_datetime_utc)
                            subtotal = sum([row[1] for row in child_space_rows.get(energy_category_id, [])], Decimal(0.0))
