...
| 表名 | 说明 | 关键字段 |
|------|------|----------|
| `tbl_users` | 用户信息 | `id`, `name`, `uuid`, `display_name`, `email`, `salt`, `password`, `is_admin`, `is_read_only`, `privilege_id`, `enterprise_space_id`, `account_expiration_datetime_utc`, `password_expiration_datetime_utc`, `failed_login_count` |
