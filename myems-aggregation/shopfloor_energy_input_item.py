"""
MyEMS Shopfloor Energy Input Item Aggregation Service

This module handles the aggregation of energy input consumption data for shopfloors by energy items.
It processes energy consumption data from various sources (meters, virtual meters, offline meters, equipment, spaces)
associated with each shopfloor and aggregates them into hourly energy consumption by items.
...
def main(logger):
    """
    Main function for shopfloor energy input item aggregation service.

    This function runs continuously and processes energy aggregation for all shopfloors
    by energy items. It uses multiprocessing to handle multiple shopfloors in parallel
...
def worker(shopfloor):
    """
    Worker function to process energy aggregation for a single shopfloor by energy items.
